...
- ``icon``      — base64 PNG of the app's small icon; passed to ``--icon``.
- ``image``     — base64 PNG of the notification's large icon; shown inline via
                  ``image-path`` hint (freedesktop notification spec, supported by KDE/GNOME).
...
async def handle_notification(conn: JibeConnection, msg: JibeMessage) -> None:
    ...
    try:
        ...
        if image_data:
            image_path = await asyncio.to_thread(_write_temp_png, image_data, "img")

        args = ["notify-send", "--app-name", display_name]
        if icon_path:
            args += ["--icon", icon_path]
        if image_path:
            args += ["-h", f"string:image-path:{image_path}"]
        args += [title, body]
