...
class ConnectionRepositoryTest {
    ...
    fun setup() {
        ...
        socketFactory = DaemonTlsSocketFactory {
            recordingSocket to trustManager
        }
    ...
    fun `start with credentials skips discovery and connects directly`() =
            testScope.runTest {
                ...
                repository.state.test {
                    ...
                    assertEquals(listOf("192.168.1.10" to 8765), recordingSocket.connectCalls)
                    cancelAndIgnoreRemainingEvents()
                }
            }
    ...
    fun `discovery finding a daemon stops discovery and connects`() =
            testScope.runTest {
                repository.state.test {
                    ...
                    assertEquals(listOf("192.168.1.20" to 8765), recordingSocket.connectCalls)
                    cancelAndIgnoreRemainingEvents()
                }
            }
                ...
                repository.state.test {
                    ...
                    val jibeMessage =
                            JibeMessage(
                                    ...
                                    payload =
                                            MessageParser.gson.toJsonTree(authMsg).asJsonObject
                            )
    ...
    fun `auth response rejected during pairing stays Authenticating with hint`() =
            testScope.runTest {
                ...
                repository.state.test {
                    ...
                    assertEquals(
                            ConnectionState.Authenticating("10.0.0.5", hint = "Invalid PIN"),
                            awaitItem()
                    )
                    cancelAndIgnoreRemainingEvents()
                }
                ...
                repository.pingResults.test {
                    ...
                    val pongMessage =
                            JibeMessage(
                                    ...
                                    payload =
                                            MessageParser.gson.toJsonTree(PingMessage()).asJsonObject
                            )
