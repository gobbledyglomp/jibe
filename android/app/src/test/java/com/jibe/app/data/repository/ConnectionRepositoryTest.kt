...
class ConnectionRepositoryTest {
    ...
    fun setup() {
        ...
        repoScope = kotlinx.coroutines.CoroutineScope(testDispatcher + kotlinx.coroutines.Job())
    ...
    fun `start with credentials skips discovery and connects directly`() =
            testScope.runTest {
                val creds = DeviceCredentials("dev_1", "finger_1", "192.168.1.10", 8765, "cert_fp")
                ...
                repository.state.test {
                    ...
                    assertEquals(ConnectionState.Connecting("192.168.1.10", 8765), awaitItem())
                    verify(exactly = 0) { discovery.startDiscovery() }
                    verify { anyConstructed<JibeWebSocketClient>().connect("192.168.1.10", 8765) }
                    cancelAndIgnoreRemainingEvents()
                }
            }
    ...
    fun `discovery finding a daemon stops discovery and connects`() =
            testScope.runTest {
                repository.state.test {
                    ...
                    // Simulate finding a daemon
                    val daemon = DiscoveredDaemon("JibeDaemon", "192.168.1.20", 8765)
                    discoveryStateFlow.value = DiscoveryState.Found(daemon)
                    advanceUntilIdle()

                    assertEquals(ConnectionState.Connecting("192.168.1.20", 8765), awaitItem())
                    verify { discovery.stopDiscovery() }
                    verify { anyConstructed<JibeWebSocketClient>().connect("192.168.1.20", 8765) }
                    cancelAndIgnoreRemainingEvents()
                }
            }
    ...
    fun `websocket connected emits Authenticating state`() =
            testScope.runTest {
                ...
                repository.state.test {
                    assertEquals(ConnectionState.Connecting("10.0.0.5", 8765), awaitItem())

                    // Simulate WS connection success
                    wsEventsFlow.emit(WebSocketEvent.Connected)
                    advanceUntilIdle()

                    assertEquals(ConnectionState.Authenticating("10.0.0.5"), awaitItem())
                    cancelAndIgnoreRemainingEvents()
                }
            }
    ...
    fun `auth response accepted saves credentials and emits Connected`() =
            testScope.runTest {
                ...
                repository.state.test {
                    ...
                    // Simulate AuthResponse(accepted=true)
                    val authMsg =
                            AuthResponse(
                    ...
                    val jibeMessage =
                            JibeMessage(
                                    ...
                                    payload = MessageParser.gson.toJsonTree(authMsg).asJsonObject
                            )
    ...
    fun `auth response rejected emits Failed state`() =
            testScope.runTest {
                ...
                repository.state.test {
                    ...
                    // Simulate AuthResponse(accepted=false)
                    val authMsg =
                            AuthResponse(
                                    type = MessageType.AUTH_RESPONSE.value,
                                    accepted = false,
                ...
                repository.state.test {
                    ...
                    val jibeMessage =
                            JibeMessage(
                                    ...
                                    payload = MessageParser.gson.toJsonTree(errMsg).asJsonObject
                            )

                    wsEventsFlow.emit(WebSocketEvent.MessageReceived(jibeMessage))
                    advanceUntilIdle()

                    assertEquals(ConnectionState.Failed("Too many failed attempts"), awaitItem())
                    cancelAndIgnoreRemainingEvents()
                }
            }
    ...
    fun `receiving ping result measures latency`() =
            testScope.runTest {
                ...
                repository.pingResults.test {
                    repository.sendPing() // Send ping, saves pingSentAt

                    // Advance time a little to simulate latency
                    advanceTimeBy(50)
                    ...
                    // 50ms should be roughly the latency, but System.currentTimeMillis() in tests
                    // might be 0.
